...
def one_dimensional_array_with_length(val: List, val_name: str, length: int, exception_class):
    if (not (type(val) is np.ndarray)):
        raise(exception_class(val_name + ' must be a list.'))
    if not all(not isinstance(item, list) for item in val):
        raise(exception_class(val_name + ' cannot contain list as its elements.'))
    if len(val) != length:
        raise(exception_class(val_name + f" must have {length} elements."))
    return val
...
def float_in_closed01_vec(val,val_name,exception_class):
    if (type(val) is np.ndarray):
        if np.issubdtype(val.dtype,np.floating) and val.ndim == 1 and np.all(val>=0) and np.all(val<=1):
            return val
    raise(exception_class(val_name + " must be a 1-dimensional numpy.ndarray whose dtype is float. Its values must be in [0,1]."))

def proba_vec(val,val_name,exception_class):
    if type(val) is np.ndarray:
        if np.issubdtype(val.dtype,np.integer) and val.ndim == 1 and abs(val.sum() - 1.) <= _EPSILON and np.all(val)>=0:
            return val.astype(float)
        if np.issubdtype(val.dtype,np.floating) and val.ndim == 1 and abs(val.sum() - 1.) <= _EPSILON and np.all(val)>=0:
            return val
    raise(exception_class(val_name + " must be a 1-dimensional numpy.ndarray with nonnegative values, and the sum of its elements must equal to 1."))
...
def nonneg_float(val,val_name,exception_class):
    if np.issubdtype(type(val),np.floating):
        if val >= 0.0:
            return val
    if np.issubdtype(type(val),np.integer):
        if val >= 0.0:
            return float(val)
    raise(exception_class(val_name + " must be non-negative (including 0.0)."))
...
def unique_list(val: List, val_name: str, exception_class):
    if len(val) == len(set(val)):
        return val
    raise(exception_class(val_name + " must be a list without duplicate elements."))

def floats_in_closedrange(val: int, val_name: float, min: float, max: float, exception_class):
    if all(min <= i <= max for i in val):
        return val
    raise(exception_class("all elements in " + val_name + f" must be in [{min},{max}]."))

def boolean(val: bool, val_name: str, exception_class):
    if type(val) is bool:
        return val
    raise(exception_class(val_name + " must be a boolean."))